...
for (product in c('Petrol', 'other_priv', 'rail_bus', 'air', 'rental_tax', 'water')){
    temp <- shp_data %>% rename(ghg=product) %>% select(MSOA11CD, AI2015_ln, ghg, geometry) %>% drop_na() %>% st_as_sf()
    # convert to sp
    mydata.sp <- as(temp, "Spatial")
    # determine the kernel bandwidth & fit GWR
    bw <- bw.gwr(ghg ~ AI2015_ln, adaptive = 30, data=mydata.sp)
    m.gwr <- gwr.basic(ghg ~ AI2015_ln, adaptive = T, data = mydata.sp, bw = bw)
